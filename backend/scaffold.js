 ...
 *
 * Run from the backend folder:   node scaffold.js
 *
 * Safe to re-run. Never overwrites an existing file.
 *
 * NOTE: each service has its own package.json and .env, but dependencies are
 * installed once from backend/ (npm workspaces): `npm run install:all`.
 */

const fs = require('fs');
const path = require('path');

const ROOT = __dirname; // this file lives in backend/
...
const ADMIN_ENTITIES = [
  ...
  'rate-master',
  'reason-codes',
  // role <-> menu <-> permission access lives in `access` (tbl_role_menu_permission);
  // the old tbl_role_menu / tbl_role_permissions tables no longer exist.
  'roles',
  'stages',
  'sub-stages',
];
...
console.log('Next:');
console.log('  1. Paste your DDL into database/schema.sql (and per-service schema.sql files)');
